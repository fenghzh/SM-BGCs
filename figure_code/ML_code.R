...
# 混淆矩阵及热力图 (confusion matrix heatmap)
rf_aug <- augment(rf_fit, new_data = bgc_data)  # 增强数据集以获取预测 (augment returns .pred_class)
conf_matrix <- conf_mat(rf_aug, truth = Category, estimate = .pred_class)
autoplot(conf_matrix, type = "heatmap")

# ROC 曲线绘制 (假设 Category 为二分类并已指定正例标签)
roc_data <- rf_aug %>% roc_curve(truth = Category, .pred_{LevelA})
autoplot(roc_data)

auc_val <- roc_auc(rf_aug, truth = Category, .pred_{LevelA})
...
p <- ggplot(acc_data, aes(x = Model, y = mean, fill = Model)) +
  ...
  labs(title = "模型准确率比较 (Accuracy Comparison)",
       ...
       x = "模型 (Model)") +
  theme_minimal()
topptx(p,"Accuracy Comparison.pptx", width =6, height = 5)
...
p <- ggplot(other_metrics, aes(x = Model, y = mean, fill = .metric)) +
  ...
  labs(title = "模型敏感性和特异性比较 (Sensitivity & Specificity Comparison)",
       ...
       x = "模型 (Model)") +
  theme_minimal()
topptx(p,"Sensitivity_Specificity.pptx", width = 6, height = 5)
...
topptx(p,"RF_Confusion_Matrix.pptx", width = 6, height = 5)
...
topptx(p,"XGB_Confusion_Matrix.pptx", width = 6, height = 5)
...
topptx(p,"SVM_Confusion_Matrix.pptx", width = 6, height = 5)
...
# PCA
bgc_data_pca <- bgc_data %>% select(-Category) %>% select(where(~ var(.) > 0))
pca_res <- prcomp(bgc_data_pca, scale. = TRUE)

autoplot(pca_res, data = bgc_data, colour = 'Category') +
  ggtitle("BGCs PCA 分布 (PCA of BGCs)")

# UMAP
umap_res <- umap::umap(bgc_data %>% select(-Category))
umap_df <- as.data.frame(umap_res$layout)
umap_df$Category <- bgc_data$Category

ggplot(umap_df, aes(x = V1, y = V2, color = Category)) +
  geom_point(size = 2, alpha = 0.8) +
  labs(title = "BGCs UMAP 分布 (UMAP of BGCs)") +
  theme_minimal()

##
##
##重要特征与环境的相关性
rf_vi <- vip::vi(rf_fit %>% extract_fit_parsnip())
...
#对 BGC 丰度做 log1p 转换
df_top <- df_top %>%
  mutate(across(all_of(top_features), ~ log1p(.x)))
...
topptx(final_plot,"RF_result.pptx", width = 8, height = 5)